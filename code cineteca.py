...
for t in range (nt):
    ...
    if Gag.shape[0]>=1 and (t)%1000==0 or t==nt:
        ...
        state=[round(100*t/nt,3),round(t*dt,5),gagpop,concgrna,vir,vlp,allostvir,round(selec,3),round(Cgags0,3),round(Cgags1,3)]
...
plt.figure(2)
plt.plot(graf[1:,1],graf[1:,3],label="gRNA/(gRNA+mRNA)")
plt.legend()
plt.title("Concentración")

plt.figure(2)
plt.plot(graf[1:,1],graf[1:,7],label="vir/(vir+vlp)")
plt.legend()

plt.figure(4)
plt.plot(graf[1:,1],graf[1:,8],label="Gags en estado compacto")
plt.legend()

plt.figure(4)
plt.plot(graf[1:,1],graf[1:,9],label="Gags en estado extendido")
plt.legend()
